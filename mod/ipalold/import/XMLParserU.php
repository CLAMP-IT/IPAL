...
function MoodleXMLQuiz2Array($xml){
	...
	foreach($elem[$level] as $elem[$level + 1]){//This is level 1. These should be questions
		...
		if($value[$level] == 'text'){$key[$level]  = $key[$level-1];}//TExt values take their values from one level up
		$qData[$n][$key[$level]] = $value[$level];
		//Starting level 2
		$na=0;//Th eindex for the answers
		foreach($elem[$level] as $elem[$level + 1]){//This is level 1. These should be questions
			$level ++;//level 2 
			$label[$level] = $elem[$level]->getName();
			if($lable[$level] == 'answer'){
