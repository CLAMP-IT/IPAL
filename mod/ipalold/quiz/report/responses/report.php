...
require_once($CFG->dirroot.'/mod/ipal/quiz/report/responses/responsessettings_form.php');
require_once($CFG->dirroot.'/mod/ipal/quiz/report/responses/responses_table.php');
...
class quiz_responses_report extends quiz_default_report {
    ...
    function display($quiz, $cm, $course) {
        ...
        $candelete = has_capability('mod/ipal/quiz:deleteattempts', $context)
        ...
        if (!$students = get_users_by_capability($context, array('mod/ipal/quiz:reviewmyattempts', 'mod/ipal/quiz:attempt'),'u.id,1','','','','','',false)) {
            $students = array();
        } else {
        ...
        if (empty($currentgroup)) {
            ...
            if (!$groupstudents = get_users_by_capability($context, array('mod/ipal/quiz:reviewmyattempts', 'mod/ipal/quiz:attempt'),'u.id,1','','','',$currentgroup,'',false)) {
                $groupstudents = array();
            } else {
        ...
        if ($students && ($attemptids = optional_param('attemptid', array(), PARAM_INT)) && confirm_sesskey()) {
            ...
            require_capability('mod/ipal/quiz:deleteattempts', $context);
