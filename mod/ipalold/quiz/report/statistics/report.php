...
require_once($CFG->dirroot.'/mod/ipal/quiz/report/statistics/statistics_form.php');
require_once($CFG->dirroot.'/mod/ipal/quiz/report/statistics/statistics_table.php');
...
class quiz_statistics_report extends quiz_default_report {
    ...
    function display($quiz, $cm, $course) {
        ...
        if (!empty($currentgroup)) {
            ...
            $groupstudents = get_users_by_capability($context, array('mod/ipal/quiz:reviewmyattempts', 'mod/ipal/quiz:attempt'),'','','','',$currentgroup,'',false);
            if (!$groupstudents){
        ...
        if (!$qid) {//main page
            ...
            if (!$this->table->is_downloading() || ($everything && $this->table->is_downloading() == 'xhtml')){
                if ($s > 1){
                    $imageurl = $CFG->wwwroot.'/mod/ipal/quiz/report/statistics/statistics_graph.php?id='.$quizstats->id;
            ...
            if (!$this->table->is_downloading()) {
                ...
                navigation_node::override_active_url(
                        new moodle_url('/mod/ipal/quiz/report.php', array('id' => $cm->id, 'mode' => 'statistics')));
    ...
    function output_individual_question_data($quiz, $question, $reporturl, $quizstats){
        ...
        require_once($CFG->dirroot.'/mod/ipal/quiz/report/statistics/statistics_question_table.php');
    ...
    function quiz_stats($nostudentsingroup, $quizid, $currentgroup, $groupstudents, $questions, $useallattempts){
        ...
        if ($s){
            require_once("$CFG->dirroot/mod/ipal/quiz/report/statistics/qstats.php");
