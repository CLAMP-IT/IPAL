...
*/
function ipal_get_questions_category( $category, $noparent=false, $recurse=true, $export=true ) {
    global $DB;
    ...
    if ($questions = $DB->get_records_select('question', "category $usql $npsql", $params, 'qtype, name')) {
        ...
        foreach($questions as $question) {
            ...
            ipal_question_bank::get_qtype($question->qtype)->get_question_options($question);
            $qresults[] = $question;
        }
    }
 ...
 */
function ipal_question_is_only_toplevel_category_in_context($categoryid) {
    global $DB;
 ...
 */
function ipal_question_can_delete_cat($todelete) {
    global $DB;
 ...
 * as of Moodle 2.0
 */
$QUESTION_EDITTABCAPS = ipal_question_edit_contexts::$caps;

/**
 * Make sure user is logged in as required in this context.
 */
function ipal_require_login_in_context($contextorid = null){
    global $DB, $CFG;
