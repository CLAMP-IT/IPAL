...
require_once($CFG->dirroot.'/mod/ipal/quiz/report/overview/overviewsettings_form.php');
require_once($CFG->dirroot.'/mod/ipal/quiz/report/overview/overview_table.php');
...
class quiz_overview_report extends quiz_default_report {
    ...
    function display($quiz, $cm, $course) {
        ...
        if (!$students = get_users_by_capability($this->context, array('mod/ipal/quiz:reviewmyattempts', 'mod/ipal/quiz:attempt'),'u.id,1','','','','','',false)) {
            $students = array();
        } else {
        ...
        if (empty($currentgroup)) {
            ...
            if (!$groupstudents = get_users_by_capability($this->context, array('mod/ipal/quiz:reviewmyattempts', 'mod/ipal/quiz:attempt'),'u.id,1','','','',$currentgroup,'',false)) {
                $groupstudents = array();
            } else {
        ...
        $reporturl = new moodle_url('/mod/ipal/quiz/report.php', $pageoptions);
        ...
        $candelete = has_capability('mod/ipal/quiz:deleteattempts', $this->context)
        ...
        if (empty($currentgroup) || $groupstudents) {
            if (optional_param('delete', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param('attemptid', array(), PARAM_INT)) {
                    require_capability('mod/ipal/quiz:deleteattempts', $this->context);
        ...
        if (!$nostudents || ($attemptsmode == QUIZ_REPORT_ATTEMPTS_ALL)) {
            ...
            if (!$table->is_downloading()) { //do not print notices when downloading
                //regrade buttons
                if (has_capability('mod/ipal/quiz:regrade', $this->context)) {
            ...
            if (!$table->is_downloading() && has_capability('mod/ipal/quiz:regrade', $this->context) && $regradedattempts) {
                $columns[] = 'regraded';
        ...
        if (!$table->is_downloading() && $showgrades) {
            if ($currentgroup && $groupstudents) {
                ...
                if ($DB->record_exists_select('quiz_grades', "userid $usql AND quiz = ?", $params)) {
                     $imageurl = "{$CFG->wwwroot}/mod/ipal/quiz/report/overview/overviewgraph.php?id={$quiz->id}&amp;groupid=$currentgroup";
            ...
            if ($DB->record_exists('quiz_grades', array('quiz'=> $quiz->id))) {
                 ...
                 $imageurl = $CFG->wwwroot.'/mod/ipal/quiz/report/overview/overviewgraph.php?id='.$quiz->id;
                 echo $OUTPUT->heading($graphname);
                 echo '<div class="graph"><img src="'.$imageurl.'" alt="'.$graphname.'" /></div>';
            }
        }
    ...
    function regrade_selected_attempts($quiz, $attemptids, $groupstudents) {
        global $DB;
        require_capability('mod/ipal/quiz:regrade', $this->context);
        if ($groupstudents) {
            list($usql, $params) = $DB->get_in_or_equal($groupstudents);
            $where = "qa.userid $usql AND ";
        } else {
