...
/**#@-*/
echo "|n<br />debug41 in local questinnlib";
/**#@+
...
echo "\n<br />debug148 in local questionlib";
require_once("./questiontype.php");
echo "\n<br />debug150 in local questionlib and getting questiontype locally";
...
$qtypenames = array('essay','multichoice','truefalse');//Debug significant change //$qtypenames= get_list_of_plugins('question/type');
foreach($qtypenames as $qtypename) {
    ...
    if (is_readable($qtypefilepath)) {echo "\n<br />debug160 in questionlib and filepath is ".$qtypefilepath;
        require_once($qtypefilepath);
    }
}
echo "\n<br />debug144 in questionlib. Lots of files have been included";
/**
...
function question_make_default_categories($contexts) {
    ...
    $toreturn = null;
    $preferredness = 0;echo "\n<br />debug1914 in questionlib in function make default categories";
    // If it already exists, just return it.
    foreach ($contexts as $key => $context) {echo "\n<br />debug1916 in questionlib in function make default categories and key is $key and contextid is ".$context->id;
        if (!$categoryrs = get_recordset_select("question_categories", "contextid = '{$context->id}'", 'sortorder, name', '*', '', 1)) {
            error('error getting category record');echo "\n<br />debug1918 there was an error";
        } else {echo "\n<br />debug1919 in questionlib in function make default categories";
            if (!$category = rs_fetch_record($categoryrs)){echo "\n<br />debug1920 in questionlib in function make default categories no category";
        ...
        }echo "\n<br />debug1934 in questionlib in function make default categories";
...
echo "\n<br />debug1941 in questionlib in function make default categories";
    if (!is_null($toreturn)) {
