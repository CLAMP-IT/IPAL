...
<title>ConcepTests from Harvard version f</title>
</head>
...
function ipal_filter_var($text){
		...
		$text = "<meta name='conceptestID' content='$conceptestID'>\n<meta name='rating' content='$rating'>\n";
...
if(isset($_POST['submit']) and ($_POST['submit'] == 'Next_CT')){$conceptestID = $conceptestID + 1;}
...
//	}
	...
	echo "</select>";
	echo " and the VocabID <input type='text' name='VocabID' value='0' size=4>";
	echo "\n<br />Title <input type='text' name='title' size=40> Rating <input type='text' name='rating' size=3>";
	echo "<hr /><hr />\n<br />";
	...
	for($a=1;$a<count($MyAnswer);$a++){
		if($a == $correctAnswer){$checked = 'checked';} else {$checked = '';}
		echo "\n<br /><input type='hidden' name='answer[$a]' value='".str_replace("'","&#039;",$MyAnswer[$a])."'><input type='radio' name='correctAnswer' value='$a' $checked> ".$MyAnswer[$a];
	}
	echo "\n<textarea cols = '120' rows='40' name='questionText'>".$questionText."</textarea>";
	echo "\n<br /><input type='hidden' name='feedback' value='".str_replace("'","&#039;",$feedback)."'>Feedback: ".$feedback;
	echo "</form>";
	}
