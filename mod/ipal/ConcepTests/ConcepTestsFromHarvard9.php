...
if(($_POST['postQuestion'] == 'certainly_now') and ($conceptestID > 0)){
  ...
  function ipal_random_string ($length=15) {
	...
	$questionFieldArray=array('category','parent','name','questiontext','questiontextformat','generalfeedback','generalfeedbackformat'
	,'defaultgrade','penalty','qtype','length','stamp','version','hidden','timecreated','timemodified','createdby','modifiedby');
	...
	$multichoiceNotNullArray = array('correctfeedback','partiallycorrectfeedback','incorrectfeedback');
		...
		$stamp = $hostname .'+'. $date .'+'.ipal_random_string(6);
		$version = $hostname .'+'. $date .'+'.ipal_random_string(6);
		$questionInsert -> category = $_POST['categpryid'];
		...
		$questionInsert ->questiontext = $_POST['questionText'];
		$questionInsert ->questiontextformat = 0;
		$questionInsert ->generalfeedback = $_POST['feedback'];
